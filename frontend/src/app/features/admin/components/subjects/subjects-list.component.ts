...
import { Subject, ExamType } from '../../../../models/test.models';
  ...
  template: `
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
      <div class="mb-6 flex justify-between items-center">
        <div>
          <h1 class="text-3xl font-bold text-gray-900">Ders Konuları</h1>
      ...
      <ng-template #content>
              ...
              <div class="flex items-center justify-between">
                <div class="flex items-center">
                  ...
                  <div class="ml-4">
                    ...
                    <div class="text-sm text-gray-500">Sınav Türü ID: {{ subject.exam_type_id.substring(0, 8) }}...</div>
                  </div>
                </div>
        ...
        <ng-template #noData>
          <div class="text-center py-12 bg-white rounded-lg shadow">
            ...
            <h3 class="mt-2 text-sm font-medium text-gray-900">Henüz ders konusu yok</h3>
            <p class="mt-1 text-sm text-gray-500">İlk ders konusunu ekleyerek başlayın.</p>
            <div class="mt-6">
              <a routerLink="/admin/subjects/new" 
...
export class SubjectsListComponent implements OnInit {
    ...
    this.adminService.listSubjects().subscribe({
      next: (response) => {
        if (response.success) {
          ...
          this.subjects.set([]);
          this.errorMessage.set(response.message || 'Ders konuları yüklenemedi.');
        }
        this.loadExamTypes();
      },
      error: (error) => {
        this.isLoading.set(false);
        this.errorMessage.set(error.error?.error?.message || 'Ders konuları yüklenirken bir hata oluştu.');
        this.subjects.set([]);
      }
    });
  }
  ...
  confirmDelete(subject: Subject): void {
    ...
    this.deleteMessage.set(`"${subject.name}" ders konusunu silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.`);
  ...
  deleteSubject(): void {
    ...
    this.adminService.deleteSubject(subject.id).subscribe({
      ...
      error: (error) => {
        this.errorMessage.set(error.error?.error?.message || 'Ders konusu silinirken bir hata oluştu.');
        this.showDeleteDialog.set(false);
      }
    });
  }
