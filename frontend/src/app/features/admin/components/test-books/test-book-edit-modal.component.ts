...
import { TestBook, ExamType, Lesson, Subject, UpdateTestBookRequest } from '../../../../models/test.models';
import { LoadingSpinnerComponent } from '../../../../shared/components/loading-spinner/loading-spinner.component';
import { ErrorMessageComponent } from '../../../../shared/components/error-message/error-message.component';

@Component({
  selector: 'app-test-book-edit-modal',
  standalone: true,
...
export class TestBookEditModalComponent implements OnInit {
  ...
  loadTestBook(bookId: string): void {
    ...
    this.adminService.getTestBook(bookId).subscribe({
      next: (response) => {
        if (response.success && response.data) {
          ...
          // Load subjects for the selected exam type and lesson
          if (book.exam_type_id && book.lesson_id) {
            this.loadSubjects(book.exam_type_id, book.lesson_id, () => {
              // Select existing subjects
              book.subject_ids.forEach(subjectId => {
                if (this.isSubjectInFilteredList(subjectId)) {
                  this.onSubjectToggle(subjectId, { target: { checked: true } } as any);
                }
              });
            });
          }
          ...
          this.errorMessage.set(response.message || 'Test kitabı yüklenemedi.');
          this.isLoading.set(false);
        }
      },
      error: (error) => {
  ...
  loadSubjects(examTypeId: string, lessonId: string, callback?: () => void): void {
  ...
  isSubjectInFilteredList(subjectId: string): boolean {
    return this.filteredSubjects().some(s => s.id === subjectId);
  }
  ...
  onSubmit(): void {
    if (this.form.valid && this.bookId) {
      ...
      this.adminService.updateTestBook(this.bookId, request).subscribe({
        next: () => {
