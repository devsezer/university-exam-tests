...
import { TestBook, Subject, PracticeTest, UpdatePracticeTestRequest } from '../../../../models/test.models';
...
@Component({
  selector: 'app-practice-test-edit-modal',
  standalone: true,
...
export class PracticeTestEditModalComponent implements OnInit {
  ...
  loadPracticeTest(): void {
    if (!this.testId) return;
    this.isLoading.set(true);
    this.adminService.getPracticeTest(this.testId).subscribe({
      next: (response) => {
        if (response.success && response.data) {
          const test = response.data;
          this.testBookId = test.test_book_id;
          this.loadTestBook(test.test_book_id);
          this.loadTestBookSubjects(test.test_book_id);
          ...
          this.errorMessage.set(response.message || 'Test yüklenemedi.');
          this.isLoading.set(false);
        }
      },
      error: (error) => {
        this.errorMessage.set(error.error?.error?.message || 'Test yüklenirken bir hata oluştu.');
        this.isLoading.set(false);
      }
    });
  }
  ...
  onSubmit(): void {
    if (this.form.valid && this.testId && this.testBookId) {
      ...
      this.adminService.updatePracticeTest(this.testId, request).subscribe({
        next: () => {
